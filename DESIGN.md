...
- Todo hub is one composition: greeting + Today hero (progress + Focus CTA) + slim status chips + a single task panel. Keep tokens and Daily Loop controls; no metric-card grid, analog clock tile, or separate Focus card on the first viewport.
...
- Calendar desktop is one composition: Calendar kicker + week title/nav, a single week time grid as the hero, then a quiet selected-day strip (tasks + reminders). No floating Upcoming dock pill, no nested board card around the whole page; yellow selection/Today accents stay.
